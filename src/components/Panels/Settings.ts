...
export default (components: OBC.Components) => {
  ...
  const onThemeChange = (event: Event) => {
    ...
    if (selector.value === undefined || selector.value === null || selector.value === 0) {
      ...
      <bim-panel>
        <bim-panel-section label="Aspecto" icon="mage:box-3d-fill">
          <bim-selector vertical @change=${onThemeChange}>
            <bim-option value="0" label="Sistema" icon="majesticons:laptop" .checked=${!html.classList.contains("bim-ui-dark") && !html.classList.contains("bim-ui-light")}></bim-option>
        ...
        <!-- </bim-panel-section>
        <bim-panel-section label="Escena" icon="tabler:world">
